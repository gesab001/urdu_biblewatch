...
var clock = (function() {
    ...
    function updateDate(prevDay) {
        ...
        var getLocalizedDate = function(){
      	  var  onLocaleSuccessCallback = function(locale) {
    	        ...
    	        d = new Date();
    	        console.log(d.toLocaleString(localeLanguage, { weekday: 'short', day: 'numeric', month: 'short'  }));        
    	        //strFullDate = arrDay[getDay] + " " + getDate + " " + arrMonth[getMonth];
    	        strFullDate = d.toLocaleString(localeLanguage, { weekday: 'short', day: 'numeric', month: 'short'  });
    ...
    function updateTime() {
        ...
        if (hour < 12) {
            strAmpm.innerHTML = "AM";
          	var hourTime = hour % 12 || 12;
        	//console.log("hourTime: " + hourTime);
            if (hourTime < 10) {
                strHours.innerHTML = "0" + hour;
            }else{
            	 strHours.innerHTML = hourTime;
            }
        } else {
        	var hourTime = hour % 12 || 12;
        	//console.log("hourTime: " + hourTime);
            if (hourTime < 10) {
                strHours.innerHTML = "0" + hourTime;
            } 
            else{
                strHours.innerHTML = hourTime;
    ...
    function loadBackgroundImage(){
    	if (tizen.preference.exists("backgroundImage")){
    		...
    		document.getElementById("digital-body").style.backgroundImage = backgroundImageURI;
    	}else{
    ...
    this.destroy = function(interval){
    	clearInterval(interval);
